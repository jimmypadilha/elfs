...
void inserir(tab_hash *h, char *var, char *escopo) {
if (!buscar(h, var, escopo))
insere(h, var, escopo);
else {
erros++;
...
void checar_variavel(tab_hash *h, char *var, char *escopo) {
if (!buscar(h, var, escopo)) {
erros++;
printf(" Variável %s não declarada %s \n", var, escopo );
    }
}
%}
